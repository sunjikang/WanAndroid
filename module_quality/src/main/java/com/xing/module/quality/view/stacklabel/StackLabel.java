package com.xing.module.quality.view.stacklabel;
...
public class StackLabel extends RelativeLayout {
    ...
    public static class Item {
        ...
        public Item(String label, int num) {
            this.label = label;
            this.num = num;
        }
    ...
    private void loadAttrs(Context context, AttributeSet attrs) {
        try {
            ...
            if (selectBackground == -1) selectBackground = R.drawable.shape_bg_red;
            if (labelBackground == -1) labelBackground = R.drawable.shape_bg_white;
            typedArray.recycle();
        } catch (Exception e) {
        }
    }
    ...
    public StackLabel setLabels(List<Item> l) {
        labels = l;
                    ...
                    public void onClick(View v) {
                        if (selectMode) {
                            for (View item : items) {
                                LinearLayout boxLabel = item.findViewById(R.id.box_label);
                                TextView txtLabel = item.findViewById(R.id.txt_label);
                                TextView txtNum = item.findViewById(R.id.txt_num);
                                boxLabel.setBackgroundResource(labelBackground);
...
//                                    selectIndexs.remove(ind);
                            ...
                            for (int index : selectIndexs) {
                                ...
                                TextView txtLabel = item.findViewById(R.id.txt_label);
                                TextView txtNum = item.findViewById(R.id.txt_num);
                                txtNum.setText(s.getNum());
                                boxLabel.setBackgroundResource(selectBackground);
                                txtLabel.setTextColor(selectTextColor);
                            }
                        }
                        if (onLabelClickListener != null)
                            onLabelClickListener.onClick(index, v, labels.get(index).getLabel());
                    }
                });
                boxLabel.setOnLongClickListener(new OnLongClickListener() {
                    @Override
