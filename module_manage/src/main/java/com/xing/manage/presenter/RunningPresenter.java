...
public class RunningPresenter extends BasePresenter<RunningContract.View> implements RunningContract.Presenter {
    ...
    public void getList(int pageNumber, int pageSize) {
        addSubscribe(create(ManageApiService.class).getRunningList(pageNumber, pageSize), new BaseObserver<ActPage<RunningBean>>(getView()) {
            @Override
            protected void onSuccess(ActPage<RunningBean> page) {
