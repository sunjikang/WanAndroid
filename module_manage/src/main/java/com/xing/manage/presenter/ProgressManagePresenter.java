...
public class ProgressManagePresenter extends BasePresenter<ProgressManageContract.View> implements ProgressManageContract.Presenter {
    /**
     * 获取 已办列表 数据
     */
    @Override
    public void getList(int pageNumber, int pageSize) {
        addSubscribe(create(ManageApiService.class).getProgressList(pageNumber, pageSize), new BaseObserver<ActPage<ProgressManageBean>>(getView()) {
            @Override
            protected void onSuccess(ActPage<ProgressManageBean> page) {
