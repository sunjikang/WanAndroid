...
public class TodoPresenter extends BasePresenter<TodoContract.View> implements TodoContract.Presenter {
    ...
    public void getTodoList(int pageNumber, int pageSize) {
        addSubscribe(create(ManageApiService.class).getTodoList(pageNumber, pageSize), new BaseObserver<ActPage<TodoBean>>(getView()) {
            @Override
            protected void onSuccess(ActPage<TodoBean> page) {
                if (isViewAttached()) {
